...
def main():
    ...
    for i in range(args.checkpoint_start, args.checkpoint_end):
        ...
        if args.out is not None and not args.out.endswith(('.pkl', '.json')):
            raise ValueError('The output file must be a pkl or a json file.')
        ...
        print("Check point loaded: ", args.checkpoint + str(i) + '.pth')
        ...
        if rank == 0:
            ...
            #MRs = validate('/home/honda/datasets/CityPersons/val_gt.json', args.out)
            MRs = validate('/home/honda/datasets/CityPersons/val_gt_for_mmdetction2.json', args.out)
